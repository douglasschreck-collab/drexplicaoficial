...
st.markdown("""
    <div class="clinic-top-bar">
        ...
        <div class="clinic-badge">RADAI SUITE v3.4 // RESILIENT HIGH AVAILABILITY</div>
    </div>
""", unsafe_allow_html=True)
...
with col2:
    ...
    if processar and notas_medicas and api_key_configured:
        with st.spinner("Compilando dados estruturados sob diretrizes internacionais..."):
            ...
            # Arquitetura de Redundância (Mecanismo Fallback contra Erro 503)
            try:
                # TENTATIVA 1: Modelo Principal de Nova Geração
                ...
                # TENTATIVA 2: Aciona o modelo alternativo se o principal estiver congestionado
                try:
                    response = client.models.generate_content(
                    ...
                    output_text = f"Ocorreu uma instabilidade severa nos servidores do Google. Erro Primário: {primary_error} // Erro Secundário: {fallback_error}"

            # Renderização segura na folha de laudo clínico
