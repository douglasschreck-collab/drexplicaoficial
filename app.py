...
st.markdown("""
    <div class="clinic-top-bar">
        ...
        <div class="clinic-badge">RADAI SUITE v3.5 // SOLID PRODUCTION</div>
    </div>
""", unsafe_allow_html=True)
...
with col2:
    ...
    if processar and notas_medicas and api_key_configured:
        with st.spinner("Compilando dados estruturados sob diretrizes internacionais..."):
            ...
            try:
                # Inicialização utilizando a espinha dorsal de produção estável do Google (gemini-3.8-flash)
                ...
                # Fallback de Contingência Local para falha total de infraestrutura do Google
                output_text = f"""
                <div class="missing-data-alert">⚠️ SISTEMA OPERANDO EM MODO DE SEGURANÇA LOCAL</div>
                Os servidores de processamento médico do Google estão enfrentando alta demanda global temporária (Erro 503). 
                
                **Seus dados inseridos estão salvos abaixo para não serem perdidos:**
                
                ```text
                {notas_medicas}
                ```
                
                *Por favor, clique no botão 'Executar Engenharia de Laudo' novamente em alguns segundos para reprocessar.*
                """
